...
# from compare_gan.src import kid_score
import kid_score
import tensorflow as tf
import keras


class KidScoreTest(tf.test.TestCase):
    ...
    def test_kid_function(self):
        with tf.Graph().as_default():
            real_data = tf.zeros((10, 4, 4, 3))
            gen_data = tf.ones((10, 4, 4, 3))
            inception_graph = self._create_fake_inception_graph()

            eval_fn = kid_score.get_kid_function(
                real_data, gen_data, 18, 19, "0_255", inception_graph.as_graph_def()
            )
            with tf.Session() as sess:
                sess.run(tf.global_variables_initializer())
                result = eval_fn(sess)
                self.assertNear(result, 0.24, 0.1)
